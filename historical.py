...
class WeatherHistoricalAnalyzer:
    """A class for analyzing historical weather data and making predictions."""
    
    def __init__(self):
        """Initialize the WeatherHistoricalAnalyzer with default settings."""
        self.models = {}
        self.scaler_y = None
    ...
    def fetch_historical_weather(_self, lat: float, lon: float, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            if hourly_data:
                ...
                _self.historical_data = df.dropna()
                return _self.historical_data
            else:
    ...
    def prepare_weather_data(self, df: pd.DataFrame = None, target_col: str = 'temperature') -> tuple:
    ...
    def predict_weather_simple(self, X=None, days: int = 5):
        """Make predictions using simple models"""
        if X is None:
            if self.historical_data is None:
                st.error("No data available for prediction")
                return {}
            X, _, _ = self.prepare_weather_data()
        ...
        if self.scaler_y is None:
            st.error("No scaler available. Please prepare data first.")
            return {}
        ...
        for model_name, model in self.models.items():
            try:
                ...
                pred_original = self.scaler_y.inverse_transform(np.array(pred_values).reshape(-1, 1)).flatten()
    ...
    def get_5day_forecast_data(self, df=None, predictions=None, days=5, temp_unit="Celsius"):
        """Return 5-day forecast data without Streamlit styling"""
        if df is None:
            df = self.historical_data
        
        if predictions is None:
            predictions = self.predict_weather_simple(days=days)
        
        if df is None or df.empty:
            st.error("No historical data available for forecast generation")
            return []
    ...
    def clear_data(self):
        """Clear all stored data and models"""
        self.historical_data = None
        self.models = {}
        self.scaler_y = None
    
    def is_ready_for_prediction(self):
        """Check if the analyzer is ready to make predictions"""
